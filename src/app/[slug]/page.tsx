...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const { page } = data;
  // Dynamic OG generation via next/og hangs on the OpenNext + CF
  // Workers stack for any non-root route — fundamental incompatibility
  // with the runtime, not a code bug. Falling back to the avatar URL
  // (static thumb). Proper fix is to pre-generate OG images at
  // page-save time and store in R2, served as static assets.
  return {
